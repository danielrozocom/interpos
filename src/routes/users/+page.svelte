  ...
  async function submitUser() {
    ...
    try {
        ...
        if (!editingUser) {
          ...
          if (checkRes.ok) {
            ...
            if (maybe && !maybe.error) {
              if (maybe && (maybe.name || maybe.balance !== undefined)) {
                throw new Error('El ID ya existe. Usa otro ID o edita el cliente existente.');
              }
            }
          ...
          if (String(modalId) !== String(editingUser.id)) {
            ...
            if (checkRes.ok) {
              ...
              if (maybe && !maybe.error) {
                if (maybe && (maybe.name || maybe.balance !== undefined)) {
                  throw new Error('El nuevo ID ya pertenece a otro cliente. Elige otro ID.');
                }
              }
            } else if (checkRes.status !== 404) {
              console.warn('Chequeo de existencia devolvió código', checkRes.status);
      ...
      if (!res.ok) {
        ...
        const serverMsg = errData?.error || 'Error guardando cliente';
      ...
      if (!saveErrors || saveErrors.length === 0) {
        saveErrors = [err?.message || 'Error al guardar cliente'];
      }
    } finally {
      savingUser = false;
    }
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <tr class="hover:bg-gray-50 transition-colors duration-150 {index % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}">
                ...
                <td class="table-cell actions-cell">
                  <div class="flex items-center justify-center gap-2">
                    ...
                    <button class="btn-sm btn-primary" title="Editar usuario" aria-label="Editar usuario" on:click={() => openUserModal(user)}>
  ...
  {#if !loading && sortedUsers.length > 0}
    <div class="pagination-info mt-4 text-center text-sm text-gray-600">
...
{#if showUserModal}
  ...
  <div class="text-sm text-gray-500 mb-3">Si cambias el ID, debe ser numérico y no puede coincidir con otro cliente existente.</div>
  {/if}
...
<style>
  ...
  @media (max-width: 640px) {
    .pagination-info { margin-top: 1rem; font-size: 0.875rem; }
  }
</style>